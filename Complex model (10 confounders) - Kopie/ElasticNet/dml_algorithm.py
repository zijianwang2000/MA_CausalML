...
def dml_ate(y_data, d_data, x_data, x_data_quad, model_g, model_m, K=5, classical=True, inference=True, alpha=0.05):
    ...
    for (train_indices, eval_indices) in skf.split(X=x_data, y=d_data):
        y_train, d_train, x_train, x_train_quad = y_data[train_indices], d_data[train_indices], x_data[train_indices], x_data_quad[train_indices]
        y_eval, d_eval, x_eval, x_eval_quad = y_data[eval_indices], d_data[eval_indices], x_data[eval_indices], x_data_quad[eval_indices]

        # Estimate outcome regression functions g_0(d)
        g_0_hat = []
        for d in [0, 1]:
            model_g[d].fit(X=x_train_quad[d_train==d], y=y_train[d_train==d])
            g_0_hat.append(model_g[d].predict(x_eval_quad))
