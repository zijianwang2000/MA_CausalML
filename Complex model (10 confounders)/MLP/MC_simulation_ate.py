...
def mc_simulation(N, n_MC=2000):
    ...
    for j in range(n_MC):
        ...
        ate_estimates[j, 1:4], sigma_estimates[j, 0], CIs[j, :2], rmses[j, :3] = dml_parallel_ate(y_data, d_data, [x_data, x_data_stand], model_g, model_m)
        ate_estimates[j, 4:], sigma_estimates[j, 1], CIs[j, 2:], rmses[j, 3:] = dml_parallel_ate(y_data, d_data, [x_data, x_data_stand], model_g, model_m, m_bounds=(0.005, 0.995))
...
with open('results_ate_mlp.pkl', 'wb') as pickle_file:
