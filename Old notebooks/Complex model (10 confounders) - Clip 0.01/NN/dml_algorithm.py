...
def dml_no_cf_ate(y_data, d_data, x_data, model_g, model_m, alpha=0.05, m_bounds=(0.01, 0.99)):
...
def dml_ate(y_data, d_data, x_data_all, model_g, model_m, K=5, alpha=0.05, classical=True, errors=True, m_bounds=(0.01, 0.99)):
...
def dml_parallel_ate(y_data, d_data, x_data_all, model_g, model_m, K=5, alpha=0.05, classical=True, errors=True, m_bounds=(0.01, 0.99)):
    # Check for transformed input features
    if isinstance(x_data_all, list):
        x_data_orig, x_data = x_data_all[0], x_data_all[1]
    else:
