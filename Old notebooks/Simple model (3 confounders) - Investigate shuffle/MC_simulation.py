...
def mc_simulation(N, n_MC=5000):
    ...
    for j in range(n_MC):
        ...
        ate_estimates[j, 2:5], sigma_estimates[j, 1], CIs[j, 2:4], rmses[j, :3] = dml_ate(y_data, d_data, x_data, model_g, model_m, K=2)
        ate_estimates[j, 5:], sigma_estimates[j, 2], CIs[j, 4:], rmses[j, 3:] = dml_ate(y_data, d_data, x_data, model_g, model_m, K=5)
